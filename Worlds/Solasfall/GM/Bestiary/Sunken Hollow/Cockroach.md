---
aliases:
  - Cockroach
tags:
  - Creature
  - Lore
---
*Periplaneta Cavensis*

Stocky and low to the ground, roughly the mass of a large cat despite its compact build. Its carapace shimmers faintly with a translucent, glass-like sheen — a thin shell of conducted [[Mana|mana]] wrapped permanently around its body. When startled badly enough, that shell can leap free of its skin entirely.

Found scavenging throughout [[The Sunken Hollow]], usually alone or in small pairs rather than true colonies.

## Stats

| Stat | Value |
|---|---|
| Blood | 8/8 |
| Sturdiness | 3/3 |
| Evasion | 12 |
| Barrier HP | 5/5 |

## [[Attributes]]

| [[Attributes\|Might]] | [[Attributes\|Grace]] | [[Attributes\|Vitality]] | [[Attributes\|Insight]] | [[Attributes\|Resolve]] | [[Attributes\|Presence]] |
|---|---|---|---|---|---|
| 2 | 2 | 3 | 1 | 1 | 0 |

## [[Movement Types|Movement]]

| Type  | Initiation | Travel Rate                                     |
| ----- | ---------- | ----------------------------------------------- |
| Walk  | 10         | 3m per [[Ticks, Priority & Resolution\|Tick]]   |
| Run   | 8          | 5m per [[Ticks, Priority & Resolution\|Tick]]   |
| Crawl | 7          | 1.5m per [[Ticks, Priority & Resolution\|Tick]] |

## [[Mana]] Shell (Passive)
The cockroach is permanently coated in a thin barrier of conducted [[Mana|mana]]. This barrier has **5 HP** and absorbs all [[Damage Types|damage]] before it reaches [[Sturdiness]] or [[Blood]]. Once reduced to 0, the barrier is gone. If the cockroach goes **5 [[Ticks, Priority & Resolution|Priority]]** without the barrier taking Priorityage Types|damage]], it is recast to full at the start of the next [[Ticks, Priority & Resolution|Priority]] step — this is an active recast, not passive regeneration.
## [[Actions]]

### Bite
*Mandibles built more for tearing detritus than flesh, but effective enough.*
**[[Damage Types|Damage]]:** 1d4 + 1 [[Damage Types|Piercing]]
**[[Ticks, Priority & Resolution|Priority]]:** 13
**[[Range & Threat|Range]]:** +0m
**Type:** Melee

### Shell Cast (Reaction)
*The cockroach hurls its own [[Mana|mana]] shell outward as a barrier in space rather than wearing it.*
As a Reaction, when targeted by a ranged [[Attacks|attack]] or spell requiring a roll, the cockroach may sacrifice its current passive [[Mana]] Shell to create a barrier of equal remaining HP in a 1m within 3m of itself. This barrier absorbs the triggering [[Attacks|attack]] and any further [[Attacks|attacks]] until depleted, or until 5 [[Ticks, Priority & Resolution|Priority]] passes without it taking [[Damage Types|Damage]], at which point it dissipates rather than recasting. The cockroach's own passive shell remains down until it next goes 5 [[Ticks, Priority & Resolution|Tick]] without taking [[Damage Types|damage]].

## Behaviour
Cockroaches are scavengers first, fighters only when cornered. They retreat toward gaps and crevices well before [[Blood]] loss becomes serious, attempting to squeeze through openings too small for pursuit. They do not coordinate with ants or other cockroaches and will flee past an ally without hesitation if it improves their own odds. A cockroach with no escape route fights only enough to create one.

Usually encountered in groups of one or two depending on conditions.

## Ecosystem Notes
See [[The Sunken Hollow]] for the full ecosystem relationships. Cockroaches scavenge opportunistically and are hunted by [[Ant Soldier|ant soldiers]], the [[Mantis]], and the [[Tarantula]] alike — they survive through avoidance rather than combat capability.
