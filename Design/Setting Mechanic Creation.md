...
- All [[Damage Types|damage]] resolves in this fixed sequence: Armour DR → [[Sturdiness]] → [[Blood]].
...
- When [[Blood]] reaches zero the character enters the dying state, gaining 1 Exhaustion per [[Rounds|Round]].
- At Exhaustion 10 the character dies permanently.

**[[Injuries|Injury]] framework**

- Critical Successes on [[Attacks|attacks]] can cause [[Injuries|wounds]] imposing –1 to Physical Tests each.
- Critical Successes that reduce [[Blood]] to zero or below cause permanent [[Injuries|injuries]] rather than [[Injuries|wounds]].
- The permanent [[Injuries|injury]] table in the core is the default. Modules may extend it but not remove the framework.
...
- [[Sanity]] thresholds and the [[Afflictions|Affliction]] framework apply in all settings unless a module explicitly opts out and replaces them with an equivalent system.
- The [[Afflictions|Affliction]] examples in the core are illustrative, not exhaustive. See the section on module-defined content below.
...
- The core defines that Armour DR exists as a flat [[Damage Types|damage]] reduction applied before [[Sturdiness]].
- The module defines: what armour exists, how it is acquired, what DR values it provides, and any secondary mechanical effects such as [[Movement Types|movement]] penalties or [[Attributes|attribute]] restrictions.
- DR values should be defined on a consistent internal scale. Cross-module comparisons are not guaranteed to be meaningful.

**[[Injuries|Injury]] restoration**

- The core defines that permanent [[Injuries|injuries]] persist until treated by advanced means.
- The module defines what those means are. Examples: magical regrowth, surgical reconstruction, prosthetic replacement.
- Modules involving augmentation or cybernetics should define how destroyed augmented body parts interact with the permanent [[Injuries|injury]] table.
...
- The core provides an [[Afflictions|Affliction]] framework and examples. The module may use those examples, extend them, or replace them entirely with setting-appropriate alternatives.
- Any module-defined [[Afflictions|Affliction]] must follow the same three-tier structure (Minor, Moderate, Severe) and must specify trigger, mechanical effect, and recovery [[Conditions|condition]] at each tier.
...
- A module may replace this with a segmented model (e.g. one pool per body segment) provided the following are preserved:
    ...
    - The Exhaustion accumulation and [[Death|death]] at Exhaustion 10 rules remain unchanged.
    - [[Injuries|Wound]] and [[Injuries|injury]] rules are adapted to function within the new model.
...
- Core [[Conditions|conditions]] are the default set. A module may add setting-specific [[Conditions|conditions]] following the same format: name, effect, and end [[Conditions|condition]].
- Core [[Conditions|conditions]] should not be removed, but a module may note that certain [[Conditions|conditions]] are rarely or never encountered in its context.
