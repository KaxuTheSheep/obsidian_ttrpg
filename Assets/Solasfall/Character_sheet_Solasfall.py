"""
character_sheet_final_Solasfall_v12.py
Final layout with correct scaling and positions with 2 new misc pages.
"""
...
def page1(c):
    ...
    for label, width in row1_items:
        draw_text(c, label, x + width/2, y_row1_label, align='center', size=10)
    ...
    for label, width in row2_items:
        draw_text(c, label, x + width/2, y_row2_label, align='center', size=10)
        add_text_field(c, f"field_health_{label.replace(' ', '_')}", x, y_row2_field, width, FIELD_HEIGHT)
        x += width + gap2

    # ---- Middle row: Travel Rate + Injuries ----
    ...
    inj_x = col2_x + travel_w + GUTTER
    inj_w = col2_w - travel_w - GUTTER
    draw_bordered_rect(c, inj_x, middle_y, inj_w, middle_h)
    draw_header(c, 'Injuries & Afflictions', inj_x + 4*mm, middle_y + middle_h - HEADER_TOP_OFFSET)

    # Permanent Injuries
    perm_y = middle_y + middle_h - 14*mm
    draw_text(c, 'Permanent Injuries', inj_x + 4*mm, perm_y, size=9)
    perm_field_h = 18*mm
    add_text_field(c, "field_perm_injuries", inj_x + 4*mm, perm_y - perm_field_h - 2*mm, inj_w - 8*mm, perm_field_h, multiline=True)

    # Afflictions 
    AFF_SHIFT = -2 * mm
    aff_y = perm_y - perm_field_h - 4*mm + AFF_SHIFT
    draw_text(c, 'Afflictions', inj_x + 4*mm, aff_y, size=9)
    aff_field_h = 18*mm
    add_text_field(c, "field_afflictions", inj_x + 4*mm, aff_y - aff_field_h - 2*mm, inj_w - 8*mm, aff_field_h, multiline=True)

    # Open and Closed
    open_y = aff_y - aff_field_h - 10*mm
    draw_text(c, 'Open', inj_x + 4*mm, open_y, size=9)
    add_text_field(c, "field_open", inj_x + 4*mm, open_y - FIELD_HEIGHT - 1*mm, 45*mm, FIELD_HEIGHT)

    closed_label_x = inj_x + 55*mm
    draw_text(c, 'Closed', closed_label_x, open_y, size=9)
    add_text_field(c, "field_closed", closed_label_x, open_y - FIELD_HEIGHT - 1*mm, 45*mm, FIELD_HEIGHT)
...
def page2(c):
    ...
    top_labels = ['Spare Trait Points', 'Essences', 'Faith', 'Languages', 'Alt investments']
    gap = (W - 2*MARGIN) / len(top_labels)
    ...
    for i, label in enumerate(top_labels):
        ...
        if label == 'Spare Trait Points':
            ...
            gild_x = fx + 4*mm
            gild_y_base = y_field - FIELD_HEIGHT - 2*mm 
        else:
            fw = 37*mm
            fh = 32*mm
    ...
    # Gilded Mints
    draw_text(c, 'Gilded Mints', gild_x - 6*mm, gild_y_base + 2*mm, size=10)  
    gild_field_y = gild_y_base - FIELD_HEIGHT
    add_text_field(c, "field_Gilded_Mints", gild_x - 6*mm, gild_field_y, 36*mm, FIELD_HEIGHT)

    # Inventory 
    inv_y = gild_field_y - FIELD_HEIGHT 
